...
typedef struct {
    rmt_encoder_t base;                    // Base encoder - standard encoder interface
    rmt_encoder_t *copy_encoder;           // Used to encode the leading and trailing pulses
    rmt_encoder_t *bytes_encoder;          // Used to encode the address and command data
    rmt_symbol_word_t nec_leading_symbol;  // NEC leading code with RMT representation
    rmt_symbol_word_t nec_trailing_symbol; // NEC trailing code with RMT representation
    int state;
} rmt_ir_nec_encoder_t;

static size_t rmt_encode_ir_nec(rmt_encoder_t *encoder, rmt_channel_handle_t channel, const void *primary_data, size_t data_size,
    ...
    const ir_nec_scan_code_t *scan_code = (const ir_nec_scan_code_t *)primary_data;
    ...
    switch (nec_encoder->state) {
        ...
        case 3: // send ending code
            encoded_symbols += copy_encoder->encode(copy_encoder, channel, &nec_encoder->nec_trailing_symbol,
                                                    sizeof(rmt_symbol_word_t), &session_state);
            if (session_state & RMT_ENCODING_COMPLETE) {
...
esp_err_t rmt_new_ir_nec_encoder(const ir_nec_encoder_config_t *config, rmt_encoder_handle_t *ret_encoder) {
    ...
    nec_encoder->nec_trailing_symbol = (rmt_symbol_word_t){
        .level0    = 1,
